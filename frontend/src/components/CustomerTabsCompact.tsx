...
interface Props {
  ...
  // When provided, component uses these instead of reading from tableOrdersStore
  eventDrivenCustomerTabs?: CustomerTab[];
  eventDrivenActiveTabId?: string | null;
  eventDrivenSetActiveTabId?: (tabId: string | null) => void;
  eventDrivenSplitTab?: (sourceTabId: string, newTabName: string, itemIndices: number[], guestId?: string) => Promise<{ success: boolean; message: string }>;
  eventDrivenMergeTabs?: (sourceTabId: string, targetTabId: string) => Promise<{ success: boolean; message: string }>;
  eventDrivenMoveItems?: (sourceTabId: string, targetTabId: string, itemIndices: number[]) => Promise<{ success: boolean; message: string }>;
  eventDrivenTableLevelItemCount?: number;
}
 ...
 * When eventDrivenCustomerTabs is provided, uses command pattern; otherwise falls back to legacy.
 */
export function CustomerTabsCompact({
  ...
  eventDrivenSetActiveTabId,
  eventDrivenSplitTab,
  eventDrivenMergeTabs,
  eventDrivenMoveItems,
  eventDrivenTableLevelItemCount
}: Props) {
  ...
  const customerTabs = eventDrivenCustomerTabs || getCustomerTabsForTable(tableNumber);
  
  // 🚀 EVENT-DRIVEN vs LEGACY: Get active customer tab
  // In event-driven mode: Find tab object from array using ID
  // In legacy mode: Use store selector
  const activeCustomerTab = eventDrivenActiveTabId !== undefined
    ? customerTabs.find(tab => tab.id === eventDrivenActiveTabId) || null
    : getActiveCustomerTab(tableNumber);
  
  // Get table-level order items count
  const tableOrder = persistedTableOrders[tableNumber];
  const tableLevelItemCount = eventDrivenTableLevelItemCount !== undefined 
    ? eventDrivenTableLevelItemCount 
    : (tableOrder?.order_items?.length || 0);
  ...
  const handleCustomerTabSelect = (customerTab: CustomerTab | null) => {
    if (eventDrivenSetActiveTabId) {
      eventDrivenSetActiveTabId(customerTab?.id || null);
    } else {
  ...
  const handleSplit = async () => {
    ...
    setIsSplitting(true);
    try {
      const result = eventDrivenSplitTab
        ? await eventDrivenSplitTab(activeCustomerTab.id, splitTabName.trim(), selectedItems)
        : await splitCustomerTab(activeCustomerTab.id, splitTabName.trim(), selectedItems);
      if (result.success) {
        setSplitTabName('');
        setSelectedItems([]);
  ...
  const handleMerge = async () => {
    ...
    setIsMerging(true);
    try {
      const result = eventDrivenMergeTabs
        ? await eventDrivenMergeTabs(activeCustomerTab.id, targetTabId)
        : await mergeCustomerTabs(activeCustomerTab.id, targetTabId);
      if (result.success) {
        setTargetTabId('');
  ...
  const handleMoveItems = async () => {
    ...
    setIsMoving(true);
    try {
      const result = eventDrivenMoveItems
        ? await eventDrivenMoveItems(activeCustomerTab.id, targetTabId, selectedItems)
        : await moveItemsBetweenTabs(activeCustomerTab.id, targetTabId, selectedItems);
      if (result.success) {
        setTargetTabId('');
        setSelectedItems([]);
