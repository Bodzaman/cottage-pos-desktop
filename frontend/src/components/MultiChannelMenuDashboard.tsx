...
// VoiceAgentCard was removed - commenting out import
...
const MultiChannelMenuDashboard: React.FC = () => {
  ...
  const getItemMinPrice = (itemId: string): string | null => {
    const variants = realtimeMenuStore.itemVariants.filter(v => v.menu_item_id === itemId && v.is_active);
  ...
  const getPriceDisplay = (itemId: string): string | null => {
    const variants = realtimeMenuStore.itemVariants.filter(v => v.menu_item_id === itemId && v.is_active);
    if (variants.length === 0) return null; // No fallback - return null when no data
  ...
  const updateMenuDataFromStore = () => {
    ...
    setMenuData({
      ...
      lastUpdated: lastUpdate ? new Date(lastUpdate).toISOString() : new Date().toISOString(),
    ...
    setMenuStatus({
      isConnected,
      lastSync: lastUpdate ? new Date(lastUpdate).toISOString() : new Date().toISOString(),
