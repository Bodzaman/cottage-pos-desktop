...
export const GoogleAddressAutocomplete: React.FC<Props> = ({ 
  ...
  useEffect(() => {
    const loadMapsConfig = async () => {
      try {
        const response = await (brain as any).get_maps_config();
      ...
      placesServiceRef.current.getDetails(
        ...
        async (place: any, status: any) => {
          if (status === window.google.maps.places.PlacesServiceStatus.OK && place) {
            ...
            // Validate delivery address if showValidation is enabled
            if (showValidation && address.postcode) {
              try {
                const deliveryResponse = await (brain as any).validate_delivery_address({
