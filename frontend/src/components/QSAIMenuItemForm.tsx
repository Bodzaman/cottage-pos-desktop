...
import { MenuItemFormData, MenuCategory, ProteinType } from '../utils/masterTypes';
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {orderContext && (
                  <div style={qsaiStyles.card} className="p-4">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-xs" style={{ color: globalColors.text.secondary }}>
                      <div>Next Order: {orderContext.nextOrder}</div>
                    </div>
                  </div>
                )}
              </CollapsibleContent>
        ...
        {(() => {
          ...
          const variantSummary = hasVariants && variants.length > 0
            ? getVariantSummary(variants, 'COLLECTION')
            : null;
...
function SpecializedFieldsRenderer({ itemType, register, watch, setValue, control }: SpecializedFieldsRendererProps) {
  switch (itemType) {
    ...
    case 'coffee_desserts':
      return (
        <div className="space-y-6">
          <div className="text-center py-8 p-6" style={qsaiStyles.card}>
