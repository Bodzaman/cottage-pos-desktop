      ...
      switch (action) {
        case 'CONFIRM':
          ...
          try {
            ...
            const assignmentResponse = await brain.get_template_assignment({ orderMode: order.orderType });
          ...
          // Update order status
          await brain.update_order_tracking_status({
            order_id: order.id,
        ...
        case 'MARK_COMPLETED':
          ...
          try {
            ...
            const assignmentResponse = await brain.get_template_assignment({ orderMode: apiOrderMode });
            const templateAssignment = await assignmentResponse.json();
          ...
          // Update order status - use COLLECTED for COLLECTION orders, DELIVERED for DELIVERY orders
          const completedStatus = order.orderType === 'COLLECTION' ? OrderStatus.COLLECTED : OrderStatus.DELIVERED;
          await brain.update_order_tracking_status({
            order_id: order.id,
