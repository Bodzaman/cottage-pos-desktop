...
/**
 * Extended MenuCategory with additional UI properties
 * Note: menu_order is an alias for display_order for backward compatibility
 */
interface ExtendedMenuCategory extends MenuCategory {
  children?: ExtendedMenuCategory[];
  item_count?: number;
  /** Alias for display_order - some parts of the codebase use this naming */
  menu_order: number;
}
...
const CategoryManagement: React.FC<CategoryManagementProps> = ({ onMenuChange, className = "" }) => {
  ...
  const handleSelectCategory = (categoryId: string) => {
    ...
    if (category) {
      setEditForm({
        ...
        menu_order: category.display_order || category.menu_order || 0,
        print_order: category.print_order || category.display_order || 0,
  ...
  const handleDeleteCategory = async (categoryId: string) => {
    try {
      ...
      const checkResponse = await brain.check_category_delete({ categoryId });
  ...
  const executeDeleteCategory = async (
    ...
    try {
      const response = await brain.safe_delete_category({
  ...
  // Process individual item assignments when deleting a category
  const processIndividualItemAssignments = async (
    categoryId: string,
    assignments: { [itemId: string]: { action: 'reassign' | 'delete'; targetCategoryId?: string } }
  ) => {
    try {
      // Group items by action
      const itemsToDelete: string[] = [];
      const itemsToReassign: { itemId: string; targetCategoryId: string }[] = [];

      for (const [itemId, assignment] of Object.entries(assignments)) {
        if (assignment.action === 'delete') {
          itemsToDelete.push(itemId);
        } else if (assignment.action === 'reassign' && assignment.targetCategoryId) {
          itemsToReassign.push({ itemId, targetCategoryId: assignment.targetCategoryId });
        }
      }

      // Process reassignments
      for (const { itemId, targetCategoryId: targetId } of itemsToReassign) {
        const { error } = await supabase
          .from('menu_items')
      ...
      toast.success(`Successfully processed ${Object.keys(assignments).length} items`);
    } catch (error) {
      console.error('Error processing individual assignments:', error);
      toast.error('Failed to process item assignments');
    }
  };
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      const response = await brain.bulk_delete_items_safe({
  ...
  const handleSaveCategory = async () => {
    ...
    try {
      ...
      const apiResponse = await brain.save_category(savePayload);
    ...
    const handleParentSelect = (categoryName: string | null) => {
      ...
      if (categoryName) {
        ...
        setFormData({
          ...
          print_order: categoryMeta?.defaultOrder || nextMenuOrder,
          print_to_kitchen: true,
          active: true,
