...
export function CustomerVariantSelector({ 
  ...
  const handleAddToCartWithCustomizations = (
    ...
    const cartCustomizations: SelectedCustomization[] = customizations?.map(c => ({
      id: c.id,
      name: c.name,
  ...
  const handleAddToCart = () => {
    ...
    if (variant) {
      // ✅ FIXED: Pass parameters in correct order (item, quantity, variant, customizations, notes)
      addToCart(item, quantity, variant, [], notes);
    } else {
      // Fallback for items with no variants - create a minimal ItemVariant
      ...
      addToCart(item, quantity, fallbackVariant, [], notes);
    }
  ...
  const handleQuickAddVariant = (variant: ItemVariant) => {
    if (!item) return;
    // ✅ FIXED: Pass parameters in correct order (item, quantity, variant, customizations, notes)
            ...
            >
              ...
              <div className="grid grid-cols-1 gap-2.5">
                {variants.map(itemVariant => {
                  ...
                  const description = itemVariant.description || item?.menu_item_description || '';
