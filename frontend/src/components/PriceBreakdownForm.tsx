...
import { Info, Truck, Utensils, Package } from "lucide-react";
...
interface PriceBreakdownFormProps {
  variantId: string;
  variantName: string;
  initialTakeawayPrice: number;
  initialDineInPrice: number | null;
  initialDeliveryPrice: number | null;
  onUpdateComplete: () => void;
}

export default function PriceBreakdownForm({
  variantId,
  variantName,
  initialTakeawayPrice,
  initialDineInPrice,
  initialDeliveryPrice,
  onUpdateComplete,
}: PriceBreakdownFormProps) {
  const [prices, setPrices] = React.useState({
    takeaway: initialTakeawayPrice,
    dineIn: initialDineInPrice ?? initialTakeawayPrice,
    delivery: initialDeliveryPrice ?? initialTakeawayPrice,
  });
    ...
    setPrices(prev => {
      ...
      if (type === "takeaway") {
        if (prev.dineIn === prev.takeaway) {
          newPrices.dineIn = numericValue;
        }
        if (prev.delivery === prev.takeaway) {
          newPrices.delivery = numericValue;
        }
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await brain.update_variant_pricing({
        variant_id: variantId,
        takeaway_price: prices.takeaway,
        dine_in_price: effectiveDineInPrice,
        delivery_price: effectiveDeliveryPrice,
      });
