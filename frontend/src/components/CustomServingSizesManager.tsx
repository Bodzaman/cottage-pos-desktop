...
export default function CustomServingSizesManager({ onServingSizeChange }: Props) {
  ...
  const loadServingSizes = async () => {
    try {
      setLoading(true);
      const response = await (brain as any).list_custom_serving_sizes({});
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const response = await (brain as any).create_custom_serving_size(createData);
      const data = await response.json();
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      const response = await (brain as any).update_custom_serving_size(
  ...
  const handleDelete = async (size: CustomServingSizeResponse) => {
    ...
    try {
      await (brain as any).delete_custom_serving_size({ servingSizeId: size.id });
