...
import { formatPrice } from 'utils/menuPricing';
...
function PaymentForm({ customerDetails, orderMode, totalAmount, selectedTime, selectedDate, onPaymentSuccess }: {
  ...
  useEffect(() => {
    const createPaymentIntent = async () => {
      try {
        ...
        // TODO: Update Brain SDK to include create_checkout_payment_intent endpoint
        const response = await (brain as any).create_checkout_payment_intent(checkoutRequest);
  ...
  const handlePayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const confirmResponse = await (brain as any).confirm_checkout_payment({
  ...
  useEffect(() => {
    const getCustomerReference = async () => {
      if (isAuthenticated && user?.id) {
        try {
          const response = await (brain as any).get_customer_reference(user.id);
  ...
  const validatePostcode = async (postcode: string) => {
    ...
    try {
      ...
      setPostcodeValidation({
        ...
        minimumOrder: result.minimum_order || null,
