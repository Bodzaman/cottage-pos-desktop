...
import { getTables, updateTableStatus } from '../utils/supabaseQueries';
...
const TableManagementModal: React.FC<TableManagementModalProps> = ({
  ...
  const handleAddTable = async () => {
    try {
      // Direct Supabase insert - no backend needed
      ...
      toast.success(`Table ${data.table_number} created successfully`);
      await loadTables();
  ...
  const handleEditTable = async () => {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .eq('table_number', editingTable.table_number);
  ...
  const handleDeleteTable = async () => {
    ...
    try {
      // Direct Supabase delete - no backend needed
      const { error } = await supabase
        .from('pos_tables')
        .delete()
        .eq('table_number', deleteTableNumber);
