...
export function VoiceOrderNotificationPanel({ compact = false }: VoiceOrderNotificationPanelProps) {
  ...
  const fetchTemplates = async () => {
    try {
      ...
      const response = await (brain as any).list_templates();
  ...
  const fetchHistory = async () => {
    try {
      ...
      const response = await (brain as any).get_notification_history({ limit: compact ? 5 : 20 });
      const data = await response.json();
  ...
  const sendTestNotification = async () => {
    try {
      ...
      const response = await (brain as any).test_template({
