...
import { DEFAULT_MIN_ORDER, DEFAULT_DELIVERY_FEE, DEFAULT_DELIVERY_SETTINGS } from "../utils/deliveryDefaults";
...
interface ExtendedDeliverySettings extends Omit<DeliverySettingsType, 'exclusion_zones'> {
  restaurant_postcode?: string;
  restaurant_coordinates?: { lat: number; lng: number };
  exclusion_zones?: ExclusionZone[];
}

// Type for the new exclusion form state
interface NewExclusionState {
  postcode: string;
  locationName: string;
  reason: string;
...
const DeliverySettings: React.FC = () => {
  ...
  type TabValue = "basic" | "map" | "zones" | "validator";
  ...
  const [newExclusion, setNewExclusion] = useState<NewExclusionState>({
    postcode: '',
    locationName: '',
    reason: '',
  ...
  const renderMap = () => {
    ...
    return (
      <div className="h-[400px] rounded-lg overflow-hidden border"
           style={{ borderColor: globalColors.border.accent }}>
        <GoogleMap
          ...
          onClick={async (e) => {
            ...
            try {
              setIsLoading(true);
              // Note: Reverse geocoding with lat/lng may be supported by backend even if not in TS types
              const response = await brain.geocode({ lat, lng } as Parameters<typeof brain.geocode>[0]);
              const data = await response.json();
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-medium mb-2 flex items-center" style={{ color: globalColors.text.primary }}>
            <Truck className="mr-2 h-5 w-5" style={{ color: globalColors.purple.primary }} />
            Delivery Management
          </h3>
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as TabValue)} className="w-full">
        ...
        <TabsContent value="basic" className="space-y-6">
          <Card style={{ ...styles.glassCard, borderColor: globalColors.border.accent }}>
            <CardHeader>
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="delivery-radius" className="flex items-center" style={{ color: globalColors.text.primary }}>
                      <MapPin className="mr-2 h-4 w-4" style={{ color: globalColors.purple.primary }} />
                      Delivery Radius
                    </Label>
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="min-order" className="flex items-center" style={{ color: globalColors.text.primary }}>
                      <Tag className="mr-2 h-4 w-4" style={{ color: globalColors.purple.primary }} />
                      Minimum Order Value
                    </Label>
                    <div className="flex items-center space-x-2">
        ...
        <TabsContent value="zones" className="space-y-6">
          <Card style={{ ...styles.glassCard, borderColor: globalColors.border.accent }}>
            ...
            <CardContent className="space-y-6">
              ...
              <Separator style={{ backgroundColor: globalColors.border.accent }} />
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="space-y-2">
                  ...
                  />
                  {isValidPostcode && (
                    <p className="text-xs" style={{ color: globalColors.purple.primary }}>Valid postcode</p>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  />
                  {isValidLocationName && (
                    <p className="text-xs" style={{ color: globalColors.purple.primary }}>Valid location</p>
                  )}
                </div>
              ...
              <div className="space-y-4">
                ...
                {(mapSettings.exclusion_zones || []).length === 0 ? (
