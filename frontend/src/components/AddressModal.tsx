...
import { MapPin, Home, Briefcase, User, Star, Trash2, Navigation, CheckCircle, AlertCircle, Loader2 } from 'lucide-react';
...
export const AddressModal: React.FC<Props> = ({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Geocode the address
      const geocodeResponse = await brain.geocode({ locationName: fullAddress });
      ...
      const deliveryResponse = await brain.validate_delivery_postcode({
  ...
  const handlePostcodeCheck = async () => {
    ...
    try {
      ...
      const geocodeResponse = await brain.geocode({
