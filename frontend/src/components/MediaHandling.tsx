...
import { ImagePlus, FileVideo, X, Eye, Upload, Info, Loader2 } from 'lucide-react';
...
import type { MediaItem as MediaLibraryItem } from 'utils/mediaLibraryUtils';
...
export function MediaHandling({
  ...
  const [aspectRatio, setAspectRatio] = useState<'square' | 'widescreen' | 'auto'>(initialAspectRatio);
  const [currentMediaTarget, setCurrentMediaTarget] = useState<'primary' | 'secondary'>('primary');
  ...
  const numericAspectRatio = aspectRatio === 'square' ? 1 : aspectRatio === 'widescreen' ? 16 / 9 : 1;

  // Get aspect ratio for media selector
  const getSelectorAspectRatio = (): 'square' | 'widescreen' | 'any' => {
    if (aspectRatio === 'auto') return 'any';
    return aspectRatio;
  };

  // Handle media selection from library
  const handleMediaSelect = (mediaItem: MediaLibraryItem): void => {
    const media: Media = {
      id: mediaItem.id,
      url: mediaItem.url,
      name: mediaItem.friendlyName || mediaItem.name,
    ...
    if (currentMediaTarget === 'secondary') {
      setSecondary(media);
    } else {
      setPrimary(media);
    }
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>): void => {
    ...
    if (file) {
      handleFileSelect(file, currentMediaTarget === 'secondary');
    }
    // Reset input
    e.target.value = '';
  };

  // Set current media target for upload/selection
  const setCurrentMedia = (target: 'primary' | 'secondary'): void => {
    setCurrentMediaTarget(target);
  };

  // Render media info section
  const renderMediaInfo = (media: Media | undefined, mediaLabel: string): React.ReactNode => {
    ...
    return (
      <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">
        <div className="flex items-center gap-2">
          ...
          {media.name && <span className="truncate max-w-[200px]">{media.name}</span>}
          {media.size && (
  ...
  // Open media selector
  const openMediaSelector = (target?: 'primary' | 'secondary'): void => {
    if (target) {
      setCurrentMediaTarget(target);
    }
  ...
  return (
    <div className={className}>
      ...
      {/* CSS for aspect ratio container is applied via className styles */}
    </div>
  );
};
