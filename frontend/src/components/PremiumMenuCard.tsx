...
const variantHasFoodDetails = (variant: ItemVariant): boolean => {
  ...
  // Handle both array and Record allergen formats
  const rawAllergens = variant.allergens;
  const allergensArray = !rawAllergens ? [] : Array.isArray(rawAllergens) ? rawAllergens : Object.keys(rawAllergens);
  const hasAllergens = allergensArray.length > 0;
...
// but props haven't changed (e.g., when other cards are updated)
export const PremiumMenuCard = React.memo(function PremiumMenuCard({
  item,
  onSelect,
  mode = 'collection',
  orderType,
  ...
  const cartItems = posStore?.currentOrder?.items || [];

  // Get addItem from cart store for OnlineOrders mode
  const addItem = useCartStore(state => state.addItem);
  
  // Helper: Get current quantity in cart for this item/variant
  // Note: cartItems may have different shapes depending on POS vs OnlineOrders context
  const getCartQuantity = (variantId?: string): number => {
    if (!cartItems || cartItems.length === 0) return 0;

    const relevantItems = cartItems.filter((cartItem: any) => {
      if (variantId) {
    ...
    return relevantItems.reduce((sum: number, cartItem: any) => sum + (cartItem.quantity || 0), 0);
  };
  ...
  // Use store variants (item-specific) or fall back to props
  const variants = useMemo(() => {
    return storeVariants || itemVariants?.filter(variant => variant.menu_item_id === item.id) || [];
  }, [storeVariants, itemVariants, item.id]);
  ...
  // 🎯 CRITICAL FIX: Check if there are multiple variants
  // We detect multi-variant items based on actual variants array length
  const isMultiVariant = variants.length > 1;
  
  // NEW: Track selected variant for multi-variant items (default to cheapest)
  // 🚀 MEMOIZED: Prevents recalculation on every render
  const activeVariants = useMemo(() => {
    return variants.filter(v => v.is_active).sort((a, b) => a.price - b.price);
  }, [variants]);
  ...
  // - POS (orderType set): Raw URLs for fastest loading
  // - Customer (no orderType): Optimized URLs for bandwidth savings
  const isPOSContext = !!orderType;
  const { imageUrl, hasImage, currentImageIndex } = useMenuItemImage({
    item,
    variants,
    context: isPOSContext ? 'pos' : 'customer',
  ...
  const getVariantDisplayNameLocal = (variant: ItemVariant): string => {
    return variant.variant_name || variant.name || 'Option';
  };
  ...
  const handleVariantNameClick = (variant: ItemVariant) => {
    ...
    const variantName = getVariantDisplayNameLocal(variant);
    ...
    if (onAddToOrder) {
      ...
      addItem(item as any, variant, qty, undefined, undefined, '');
  ...
  // Image URLs are now provided by useMenuItemImage hook above
  // For card view, use imageUrl directly (already optimized based on context)
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    if (isMultiVariant && selectedVariant) {
      // ✅ FIX: Pass complete variant object instead of stripped-down version
      // This preserves variant_name, price_delivery, and all other fields
      addItem(item as any, selectedVariant, quantity, undefined, undefined, '');
      ...
      addItem(item as any, singleVariant, quantity, undefined, undefined, '');
  ...
  const handleAddToCartWithCustomizations = (
    ...
    // Add to cart with customizations (updated signature: item, variant, quantity, customizations, orderMode, notes)
    addItem(item as any, variantForCart, quantity, cartCustomizations, undefined, notes || '');
      ...
      >
                    ...
                    <HoverCard openDelay={200}>
                      ...
                      >
                        ...
                        {variantsWithFoodDetails.map((variant) => {
                          ...
                          // Handle both array and Record allergen formats
                          const rawAllergens = variant.allergens;
                          const allergens: string[] = !rawAllergens ? [] : Array.isArray(rawAllergens) ? rawAllergens : Object.keys(rawAllergens);
        ...
        {orderType ? (
          <StaffVariantSelector
            ...
            orderType={orderType}
            onAddToOrder={(orderItem) => {
              if (onAddToOrder) onAddToOrder(orderItem);
  ...
  return (
    ...
    >
      {/* Background Image - Only render if item has images */}
      {hasImage ? (
