...
export function MenuCorpusDiagnosticsPanel() {
  ...
  const runSqlTest = async () => {
    ...
    try {
      const response = await (brain as any).test_sql_direct_helper();
  ...
  const populateSampleData = async () => {
    ...
    try {
      // TODO: Update Brain SDK to include populate_sample_menu_data_v2_helper endpoint
      const response = await (brain as any).populate_sample_menu_data_v2_helper();
      const data = await response.json();
  ...
  const runComprehensiveTest = async () => {
    ...
    try {
      const response = await (brain as any).run_comprehensive_test_helper();
      const data = await response.json();
