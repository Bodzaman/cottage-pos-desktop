...
import { RefreshCw, Save, Database, Clock, CheckCircle, AlertTriangle, Calendar, Download, Upload, X, BookOpen, FileJson, Info, Zap } from 'lucide-react';
...
export const MenuKnowledgeBase: React.FC<MenuKnowledgeBaseProps> = ({ className }) => {
  ...
  const fetchMenuCorpus = async () => {
    try {
      setIsLoading(true);
      const response = await brain.get_menu_corpus({
        headers: {
      ...
      if (data.success) {
        ...
        setSampleEntries((data as any).corpus_data || []);
        setDeltaChanges((data as any).delta_changes || null);
  ...
  const fetchVersionHistory = async () => {
    try {
      const response = await (brain as any).get_menu_versions({
        headers: {
          Authorization: `Bearer qsai-voice-auth-2025`
        }
      });
  ...
  const fetchSyncSchedule = async () => {
    try {
      ...
      const response = await (brain as any).get_sync_schedule({
        headers: {
          Authorization: `Bearer qsai-voice-auth-2025`
        }
      });
  ...
  const saveSyncSchedule = async () => {
    try {
      ...
      const response = await (brain as any).update_sync_schedule(syncSchedule, {
        headers: {
          Authorization: `Bearer qsai-voice-auth-2025`
        }
      });
  ...
  const handleSyncMenu = async () => {
    try {
      setIsSyncing(true);
      const response = await (brain as any).sync_menu_data_wrapper({
        force: true
      }, {
        headers: {
          Authorization: `Bearer qsai-voice-auth-2025`
      ...
      toast.error('Failed to sync menu data with Ultravox');
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  const getNextRunText = () => {
    ...
    try {
      ...
      // If next run is today
      if (nextRun.toDateString() === now.toDateString()) {
        return `Today at ${nextRun.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`;
      }
      ...
      tomorrow.setDate(tomorrow.getDate() + 1);
      if (nextRun.toDateString() === tomorrow.toDateString()) {
        return `Tomorrow at ${nextRun.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`;
      }
            ...
            <TabsContent value="formatted" className="mt-0">
              <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
                <div className="flex items-center text-white mb-4">
                  ...
                  <h3 className="font-medium">Ultravox Corpus Format</h3>
                </div>
    ...
    "dietary_options": [      // Optional dietary accommodations
      "string", ...           // e.g., "Vegan options", "Gluten-free"
                    ...
                    <div>
                      ...
                      <p className="text-gray-400 text-sm">
                        This corpus is automatically integrated with your Ultravox voice agent.
                      ...
                      <div className="mt-3 bg-indigo-900/20 border border-indigo-800/30 p-3 rounded-md">
                        <div className="flex gap-2">
                          ...
                          <p className="text-indigo-300 text-xs">
                            The Ultravox system provides natural language understanding capabilities
