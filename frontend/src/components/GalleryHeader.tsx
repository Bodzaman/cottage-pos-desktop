...
export function GalleryHeader({ assetType, count, onShowHelp }: GalleryHeaderProps) {
  ...
  const getDisplayInfo = () => {
    const iconStyle = { color: colors.purple.primary };
    switch (assetType) {
      case 'menu-item':
        return {
          icon: <Image className="h-5 w-5" style={iconStyle} />,
          text: count === 1 ? 'Menu Image' : 'Menu Images',
        };
      case 'ai-avatar':
        return {
          icon: <Bot className="h-5 w-5" style={iconStyle} />,
          text: count === 1 ? 'AI Avatar' : 'AI Avatars',
        };
      case 'general':
        return {
          icon: <FolderOpen className="h-5 w-5" style={iconStyle} />,
          text: count === 1 ? 'General Asset' : 'General Assets',
        };
      case 'all':
      default:
        return {
          icon: <LayoutGrid className="h-5 w-5" style={iconStyle} />,
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-lg font-semibold" style={{ color: colors.text.primary }}>
          Showing{' '}
          <span style={{ color: colors.purple.primary }}>{count}</span>{' '}
          {text}
        </h2>
      </div>
      {onShowHelp && (
        <Button
          ...
          size="sm"
          onClick={onShowHelp}
          className="hover:bg-[rgba(124,58,237,0.1)] transition-all duration-200"
