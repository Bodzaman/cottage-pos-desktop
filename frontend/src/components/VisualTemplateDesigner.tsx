...
import {
  ...
  DollarSign, MessageSquare, Minus, Clock, QrCode, ClipboardCopy
} from 'lucide-react';
import {
  ...
  FontFamily, FontWeight, TextAlign, TemplateType, ConditionalVisibility,
  generateElementId, ElementType, ElementData
} from 'utils/visualTemplateTypes';
import { getCalibratedConfig } from 'utils/thermalCalibration';
import { parseAndFormatEnhanced, getSampleFormattedContent } from 'utils/enhancedThermalFormatting';
...
// Extended Fabric object with custom data property
...
export default function VisualTemplateDesigner({ 
  ...
  useEffect(() => {
    if (canvasRef.current && !fabricCanvasRef.current) {
      ...
      canvas.on('selection:created', (e) => {
        const activeObject = e.selected?.[0] as ExtendedFabricObject | undefined;
        if (activeObject && activeObject.data) {
          setSelectedElementId(activeObject.data.id || null);
        }
      });
  ...
  const sendObjectToBack = (canvas: fabric.Canvas, object: any) => {
    try {
      ...
      if (typeof canvas.sendObjectToBack === 'function') {
        ...
        // @ts-expect-error Fabric.js v6 insertAt signature differs from types
  ...
  const updateGrid = () => {
    ...
    const existingGrid = canvas.getObjects().filter(obj => (obj as ExtendedFabricObject).data?.isGrid);
    existingGrid.forEach(obj => canvas.remove(obj));

    // Remove existing safe area guides
    const existingSafeArea = canvas.getObjects().filter(obj => (obj as ExtendedFabricObject).data?.isSafeArea);
  ...
  const addElementToCanvas = (paletteElement: PaletteElement) => {
    ...
    const textObj = new fabric.Text(paletteElement.default_data.text || '', {
      left: 50,
      top: 50,
      fontSize: paletteElement.default_style.font_size || 16,
      fontFamily: paletteElement.default_style.font_family || 'Arial',
      fontWeight: paletteElement.default_style.font_weight || 'normal',
      fill: paletteElement.default_style.color || '#000000',
      textAlign: paletteElement.default_style.text_align || 'left',
      data: { id: elementId, type: paletteElement.type }
    });
    ...
    if (currentTemplate) {
      const newElement: CanvasElement = {
        id: elementId,
        type: paletteElement.type as ElementType,
        x: 50,
        y: 50,
        width: 200,
        height: 30,
        visible: true,
        locked: false,
        zIndex: currentTemplate.canvas_elements.length,
        style: {
          font_family: (paletteElement.default_style.font_family || 'Arial') as FontFamily,
          font_size: paletteElement.default_style.font_size || 16,
          font_weight: (paletteElement.default_style.font_weight || 'normal') as FontWeight,
          text_align: (paletteElement.default_style.text_align || 'left') as TextAlign,
          color: paletteElement.default_style.color || '#000000',
          background_color: 'transparent',
          padding_top: paletteElement.default_style.padding_top || 0,
          padding_bottom: paletteElement.default_style.padding_bottom || 0,
          padding_left: paletteElement.default_style.padding_left || 0,
          padding_right: paletteElement.default_style.padding_right || 0,
          margin_top: paletteElement.default_style.margin_top || 0,
          margin_bottom: paletteElement.default_style.margin_bottom || 0
        },
  ...
  const updateElementProperty = (property: string, value: any) => {
    ...
    if (currentTemplate && selectedElementId) {
      ...
      if (elementIndex !== -1) {
        ...
        if (property === 'text') {
          element.data = { ...element.data, text: value };
        } else {
          element.style = {
            ...element.style,
            [property]: value
  ...
  const saveCanvasToTemplate = () => {
    ...
    canvas.getObjects().forEach((obj, index) => {
      ...
      if (extObj.data?.isGrid || extObj.data?.isSafeArea) return;

      if (obj.type === 'text' && extObj.data?.id) {
        const textObj = obj as Text;

        const element: CanvasElement = {
          id: extObj.data.id,
          type: (extObj.data.type || 'footer') as ElementType,
          x: textObj.left || 0,
          y: textObj.top || 0,
          ...
          zIndex: index,
          style: {
            font_family: (textObj.fontFamily || 'Arial') as FontFamily,
            font_size: textObj.fontSize || 16,
            font_weight: (textObj.fontWeight || 'normal') as FontWeight,
            text_align: (textObj.textAlign || 'left') as TextAlign,
          ...
          data: {
            text: extObj.data?.text as string | undefined,
            placeholder: extObj.data?.placeholder as string | undefined,
            dynamic_field: extObj.data?.dynamic_field as string | undefined,
            conditional_visibility: extObj.data?.conditional_visibility as ConditionalVisibility | undefined
          }
        };
  ...
  const loadCanvasFromTemplate = (template: ReceiptTemplate) => {
    ...
    const objectsToRemove = canvas.getObjects().filter(obj => {
      const extObj = obj as ExtendedFabricObject;
      return !extObj.data?.isGrid && !extObj.data?.isSafeArea;
    });
    objectsToRemove.forEach(obj => canvas.remove(obj));

    // Add template elements to canvas
    template.canvas_elements.forEach(element => {
      const textObj = new fabric.Text(element.data?.text || '', {
        left: element.x,
        top: element.y,
        ...
        data: { id: element.id, type: element.type, ...element.data }
      });
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {selectedElementId ? (
            <div className="space-y-6">
                      ...
                      {[
                        {
                          ...
                          icon: '🏪',
                          style: {
                            font_family: 'Arial' as FontFamily,
                            font_size: 16,
                        ...
                        },
                        {
                          ...
                          icon: '📝',
                          style: {
                            font_family: 'Courier' as FontFamily,
                            font_size: 10,
              ...
              {(selectedElement?.data?.image_url !== undefined || selectedElement?.data?.qr_url !== undefined) && (
                <div className="pt-6 border-t border-gray-700/30">
                  <h3 className="text-sm font-medium text-gray-300 mb-4 flex items-center">
                    {selectedElement?.data?.qr_url !== undefined ? (
                    ...
                    {selectedElement?.data?.qr_url !== undefined ? 'QR Code Settings' : 'Image Settings'}
                  </h3>
                              ...
                              if (selectedElement) {
                                const updatedElements = currentTemplate?.canvas_elements.map(element => {
                                  if (element.id === selectedElementId) {
                                    return {
                                      ...element,
                                      data: { ...element.data, qr_url: e.target.value }
                                    };
                                  }
                                  return element;
                                });

                                if (currentTemplate && updatedElements) {
                                  setCurrentTemplate({
                                    ...currentTemplate,
                                    canvas_elements: updatedElements
                                  });
                            ...
                            if (selectedElement) {
                              const updatedElements = currentTemplate?.canvas_elements.map(element => {
                                if (element.id === selectedElementId) {
                                  return {
                                    ...element,
                                    data: { ...element.data, qr_size: value }
                                  };
                                }
                                return element;
                              });
                            ...
                            if (selectedElement) {
                              const updatedElements = currentTemplate?.canvas_elements.map(element => {
                                if (element.id === selectedElementId) {
                                  return {
                                    ...element,
                                    data: { ...element.data, error_correction: value }
                                  };
                                }
                                return element;
                              });
                  ...
                  {selectedElement?.data?.image_url !== undefined && (
                    <div className="space-y-4">
                      {/* Image Upload */}
                      <div>
                            ...
                            if (selectedElement) {
                              const updatedElements = currentTemplate?.canvas_elements.map(element => {
                                if (element.id === selectedElementId) {
                                  return {
                                    ...element,
                                    data: { ...element.data, alt_text: e.target.value }
                                  };
                                }
                                return element;
                              });
                            ...
                            if (selectedElement) {
                              const updatedElements = currentTemplate?.canvas_elements.map(element => {
                                if (element.id === selectedElementId) {
                                  return {
                                    ...element,
                                    data: { ...element.data, thermal_optimized: !!checked }
                                  };
                                }
                                return element;
                              });
