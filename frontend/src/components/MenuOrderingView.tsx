...
export function MenuOrderingView() {
  ...
  // Signature matches CustomerVariantSelector's addToCart: (item, quantity, variant, customizations?, notes?)
  const handleAddToCart = (item: MenuItem, quantity: number, variant: ItemVariant, customizations?: SelectedCustomization[], notes?: string) => {
    ...
    // addItem signature: (item, variant, quantity, customizations?, orderMode?, notes?)
    // Map SelectedCustomization (price_adjustment) to CartCustomization (price) format
    const mappedCustomizations = customizations?.map(c => ({
      id: c.id,
      name: c.name,
      price: c.price_adjustment,
      group: c.group
    }));
    addItem(item as any, variant, quantity, mappedCustomizations, orderMode, notes);
