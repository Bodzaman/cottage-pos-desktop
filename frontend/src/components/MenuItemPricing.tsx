...
export function MenuItemPricing({ pricing, hasVariants, onChange, errors }: Props) {
  ...
  // 🟠 DEBUG: Log when MenuItemPricing renders
  useEffect(() => {
    console.log('🟠 [MenuItemPricing] RENDERED with pricing:', {
      price: pricing.price,
      price_takeaway: pricing.price_takeaway,
      price_dine_in: pricing.price_dine_in,
      price_delivery: pricing.price_delivery,
      hasVariants,
      onChangeExists: typeof onChange === 'function'
    });
  }, [pricing, hasVariants, onChange]);
  ...
  const handlePriceChange = (field: keyof PricingData, value: number) => {
    // 🔵 DEBUG: Log price change to trace variant price updates
    console.log('🔵 [MenuItemPricing] handlePriceChange called:', {
      field,
      value,
    ...
    console.log('🔵 [MenuItemPricing] Calling onChange with:', newPricing);
