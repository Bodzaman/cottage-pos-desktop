...
export function CustomerPortalSection({ isOpen, onClose, className = '' }: CustomerPortalSectionProps) {
  ...
  const loadCustomerData = async () => {
    ...
    try {
      ...
      try {
        const ordersResponse = await brain.get_user_orders({ userId: user?.id });
      ...
      try {
        const favoritesResponse = await brain.get_user_favorites({ user_id: user?.id });
      ...
      try {
        const addressesResponse = await brain.get_customer_addresses({ customerId: profile?.id || user?.id || '' });
  ...
  const handleAddAddress = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await brain.create_customer_address({
      ...
      if (responseData.success) {
        ...
        toast.info(responseData.message || 'Address feature coming soon');
      }
    } catch (error) {
  ...
  const handleRemoveFavorite = async (itemId: string) => {
    try {
      const response = await brain.remove_favorite({
        customer_id: profile?.id || '',
        menu_item_id: itemId
      });
      const responseData = await response.json();
              ...
              <div>
                ...
                {profile?.customer_reference_number && (
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    Customer Reference: <span className="font-mono font-semibold text-[#8B1538]">{profile.customer_reference_number}</span>
                  </p>
                )}
              </div>
            </div>
