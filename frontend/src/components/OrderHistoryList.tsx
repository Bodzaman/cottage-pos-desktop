...
// Local order item interface matching OrderCard/OrderDetailModal expectations
...
// Local order type matching OrderCard/OrderDetailModal expectations
...
interface OrderHistoryListProps {
  onOrderSelect?: (order: OrderHistoryData) => void;
}
...
export function OrderHistoryList({ onOrderSelect }: OrderHistoryListProps) {
  ...
  const [selectedOrder, setSelectedOrder] = useState<OrderHistoryData | null>(null);
  ...
  // Handle view details
  const handleViewDetails = (order: OrderHistoryData) => {
  ...
  const fetchOrders = async (page: number = 1) => {
    ...
    try {
      ...
      const mockOrders: OrderHistoryData[] = [
        {
  ...
  useEffect(() => {
    fetchOrders(1);
  }, [user, orderType, orderStatus, dateRangeFilter]);
        ...
        <>
          {orders.length === 0 ? (
            <Alert className="bg-gray-800 border-gray-700 text-gray-300">
              <AlertDescription>
                {searchTerm || orderType !== 'ALL' || orderStatus !== 'ALL' || dateRangeFilter !== 'all'
