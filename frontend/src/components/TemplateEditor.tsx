...
export default function TemplateEditor({ templateId, templateType = 'customer_receipt', onSave, onCancel }: TemplateEditorProps) {
  ...
  const loadTemplate = async () => {
    ...
    try {
      setIsLoading(true);
      const response = await (brain as any).get_template({ templateId });
  ...
  const generatePreview = async () => {
    ...
    try {
      ...
      const response = await (brain as any).preview_template({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (templateId) {
        ...
        response = await (brain as any).update_template({ templateId }, templateData);
      } else {
        // Create new template
        response = await (brain as any).create_template(templateData);
      }
