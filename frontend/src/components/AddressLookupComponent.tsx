...
export const AddressLookupComponent: React.FC<Props> = ({ 
  ...
  const handlePostcodeCheck = async () => {
    ...
    try {
      ...
      const response = await brain.geocode({ postcode: postcode.trim().toUpperCase() });
  ...
  const handleConfirmAddress = async () => {
    ...
    try {
      ...
      const response = await brain.geocode({ locationName: fullAddress });
      const data = await response.json();
      ...
      if (data.success && data.coordinates) {
        ...
        setMapData({
          ...
          locationName: data.locationName || fullAddress,
        });
