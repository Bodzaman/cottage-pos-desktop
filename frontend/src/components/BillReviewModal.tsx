...
export function BillReviewModal({
  ...
  // Calculate totals (with null safety to prevent .toFixed() errors)
  const subtotal = calculateSubtotal(orderItems) || 0;
  const tax = 0; // VAT is already included in menu prices
  const total = subtotal || 0; // No VAT addition since prices are VAT-inclusive
