...
export default function PrinterConfig() {
  ...
  const checkServiceStatus = async () => {
    try {
      // TODO: Update Brain SDK to include get_service_status endpoint
      const response = await (brain as any).get_service_status();
  ...
  const checkPrinterStatus = async () => {
    setIsChecking(true);
    try {
      // TODO: Update Brain SDK to include check_voice_api_health2 endpoint
      const response = await (brain as any).check_voice_api_health2(); // Use any endpoint as health check
  ...
  const testPrint = async (type: 'kitchen' | 'customer') => {
    try {
      ...
      const response = await brain[endpoint](testData);
      const result = await response.json();
