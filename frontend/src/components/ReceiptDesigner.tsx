...
// Extended ReceiptTemplate for local use with additional UI-specific properties
interface ReceiptTemplate extends Omit<TemplateResponse, 'is_user_template'> {
  is_default: boolean;
...
export function ReceiptDesigner({ onRefresh }: ReceiptDesignerProps) {
  ...
  // Helper to convert TemplateResponse to local ReceiptTemplate
  const convertToReceiptTemplate = (template: TemplateResponse): ReceiptTemplate => ({
    ...template,
    is_default: !template.is_user_template, // System templates are defaults
    sections: template.design_data?.sections as ReceiptSection[] | undefined,
    settings: template.design_data?.settings as Record<string, unknown> | undefined
  });
  ...
  const loadTemplates = async () => {
    ...
    try {
      setLoading(true);
      const response = await brain.list_receipt_templates({ user_id: user.id });
      const data = await response.json() as TemplateListResponse;

      if (data.success) {
        const convertedTemplates = (data.templates || []).map(convertToReceiptTemplate);
  ...
  const loadTemplate = async (templateId: string) => {
    ...
    try {
      const response = await brain.get_receipt_template({ templateId, user_id: user.id });
      const data = await response.json() as TemplateGetResponse;

      if (data.success && data.template) {
        setSelectedTemplate(convertToReceiptTemplate(data.template));
      }
    } catch (error) {
  ...
  const testPrintTemplate = async () => {
    ...
    try {
      ...
      // Build print data for local helper app
      const printData = {
      ...
      try {
        ...
        console.log('Helper response:', helperResult);
        toast.success('Test receipt sent to printer successfully!');
      } catch (helperError) {
        console.log('Local helper not available, skipping direct print');
        toast.info('Local print helper not available. Preview generated successfully.');
      }
  ...
  const resetToDefaults = async () => {
    try {
      setLoading(true);
      // Reload templates from server - system templates are always available
      await loadTemplates();
...
function TemplateSelector({
  ...
  const duplicateTemplate = async () => {
    ...
    try {
      ...
      const response = await brain.create_receipt_template(createRequest);
  ...
  const saveTemplate = async () => {
    if (!editingTemplateName.trim() || !userId) {
    ...
    try {
      if (isCreatingTemplate) {
        ...
        const response = await brain.update_receipt_template(
  ...
  const deleteTemplate = async () => {
    if (!selectedTemplate || selectedTemplate.is_default || !userId) return;

    try {
      const deleteRequest: TemplateDeleteRequest = { user_id: userId };
