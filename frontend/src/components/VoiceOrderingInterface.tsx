...
import { Mic, MicOff, Clock, ShoppingCart, Sparkles, CheckCircle2, X, ArrowRight, User, CreditCard, Edit } from 'lucide-react';
...
export default function VoiceOrderingInterface({ user, selectedAgent, onVoiceCartUpdate }: VoiceOrderingInterfaceProps) {
  ...
  const triggerHangUpWarning = async () => {
    ...
    try {
      const response = await (brain as any).trigger_hangup_warning({
  ...
  const startVoiceOrdering = async () => {
    try {
      const sessionRequest: CreateAuthenticatedVoiceCartSessionRequest = {
        user_id: user.id,
      };

      const response = await (brain as any).create_authenticated_voice_cart_session(sessionRequest);
      ...
      if (data.success) {
        ...
        if (data.ultravox_session_url) {
          // Ultravox integration placeholder
        }
      } else {
  ...
  const clearVoiceCartSession = async (sessionId: string) => {
    try {
      const response = await (brain as any).clear_cart_session({ sessionId });
      const data = await response.json();

      if (data.success) {
        console.log("Cart session cleared successfully");
      } else {
        console.error("Failed to clear cart session:", data);
      }
    } catch (error) {
      ...
      switch (decision) {
        case 'proceed':
          ...
          if (finalCart && finalCart.items.length > 0) {
            ...
            navigate('/checkout', { state: { cartItems: checkoutItems, total: finalCart.total } });
