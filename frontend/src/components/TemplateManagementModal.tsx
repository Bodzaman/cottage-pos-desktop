...
export function TemplateManagementModal({
  ...
  const loadTemplates = async () => {
    try {
      ...
      if (templatesResult.success && templatesResult.data) {
        // Transform Template[] to TemplateLibraryItem[] (no preview loading for now)
        const templatesWithPreviews: TemplateLibraryItem[] = templatesResult.data.map((template) => ({
          id: template.id,
          name: template.metadata.name,
          description: template.metadata.description,
          created_at: '', // Will be populated from design_data if available
          updated_at: '',
          businessName: template.design_data?.businessName,
          templateType: template.design_data?.receiptFormat,
          foh_preview_url: undefined,
          kitchen_preview_url: undefined
        }));
      ...
      if (assignmentsResult.success && assignmentsResult.data) {
        ...
        // Handle assignments as object keyed by order_mode
  ...
  const loadTemplateDetails = async (templateId: string) => {
    try {
      setIsLoading(true);
      const result = await getReceiptTemplate(templateId);

      if (result.success && result.data) {
        // Transform Template to local Template format
        const template: Template = {
          id: result.data.id,
          name: result.data.metadata.name,
          description: result.data.metadata.description,
          design_data: result.data.design_data,
          created_at: '', // Not available in direct query
          updated_at: ''
        };
