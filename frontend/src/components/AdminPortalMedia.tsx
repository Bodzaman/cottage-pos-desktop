  ...
  useEffect(() => {
    const initializeLibrary = async () => {
      ...
      try {
        ...
        toast.error('Failed to check media library status. Attempting to load existing assets.');
      } finally {
              ...
              <div className="flex flex-col lg:flex-row gap-6">
                ...
                <div className="flex-1">
                  {selectedAsset.type === 'video' ? (
                    <div className="aspect-video rounded-lg overflow-hidden" style={{ backgroundColor: colors.background.primary }}>
                      <video 
        ...
        onConfirmDelete={async (asset: MediaItem, replacementAssetId?: string) => {
          try {
            ...
            if (replacementAssetId) {
              // Replace asset references first using direct Supabase query
              const replaceResult = await replaceAssetInMenuItems(asset.id, replacementAssetId);

              if (!replaceResult.success) {
                throw new Error('Failed to replace asset references');
              }
            } else {
              // Remove references using direct Supabase query
              const removeResult = await removeAssetReferences(asset.id);

              if (!removeResult.success) {
                console.warn('Failed to remove asset references, continuing with deletion');
              }
            }
