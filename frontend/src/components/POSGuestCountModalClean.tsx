...
interface Props {
  ...
  tables?: RestaurantTable[]; // NEW: Optional event-driven table data
  tablesLoading?: boolean; // NEW: Optional loading state for external tables
}
...
export function POSGuestCountModalClean({ 
  ...
  initialGuestCount,
  tableCapacity = 4,
  tables: externalTables, // NEW: Optional event-driven table data
  tablesLoading: externalTablesLoading // NEW: Optional loading state for external tables
}: Props) {
  // State management
   ...
   * NEW: Use external tables if provided (event-driven), otherwise fetch from API (legacy)
   */
  useEffect(() => {
    if (isOpen) {
      ...
      // NEW: Use external tables if provided (event-driven architecture)
      if (externalTables && externalTables.length > 0) {
        
        // Convert RestaurantTable[] to TableData[]
        const convertedTables: TableData[] = externalTables.map(table => ({
          number: parseInt(table.table_number),
          capacity: table.capacity,
          isAvailable: table.status === 'VACANT', // Map VACANT status to isAvailable
          status: table.status
        }));
        
        setAllTables(convertedTables);
        setIsLoadingTables(externalTablesLoading || false);
        
        // Update actual table capacity from external data
        const currentTable = convertedTables.find(t => t.number === tableNumber);
        if (currentTable) {
          setActualTableCapacity(currentTable.capacity);
        }
      } else {
        // LEGACY: Fall back to fetching from brain API
        fetchTables();
      }
    }
  }, [isOpen, initialGuestCount, tableCapacity, tableNumber, externalTables, externalTablesLoading]);
