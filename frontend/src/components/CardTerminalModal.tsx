...
export function CardTerminalModal({
  ...
  const initializeAdyenDropin = async () => {
    try {
      ...
      // TODO: Update Brain SDK to include get_payment_config endpoint
      const configResponse = await (brain as any).get_payment_config();
      ...
      const sessionResponse = await (brain as any).create_payment_session({
