...
interface EnhancedMenuItem extends MenuItem {
  allergens?: string[];
  ingredients?: string[];
  long_description?: string | null; // Legacy description field
}
...
const MenuManagementDialog: React.FC<MenuManagementDialogProps> = ({ isOpen, onClose }) => {
  ...
  const loadSetMeals = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Map SetMealListResponse to SetMeal type - add missing fields with defaults
        const mappedSetMeals: SetMeal[] = (data || []).map((item: { id: string; code: string; name: string; description?: string | null; hero_image_url?: string | null; set_price: number; active: boolean; individual_items_total: number; savings: number; created_at: string; updated_at: string }) => ({
          ...item,
          items: [], // SetMealListResponse doesn't include items array
          item_type: 'set_meal' as const
        }));
        setSetMeals(mappedSetMeals);
      }
    } catch (error) {
  ...
  const handleStartEditing = (item: EnhancedMenuItem) => {
    ...
    setEditFormData({
      ...
      description: item.description, // Use description instead of long_description
  ...
  const handleDuplicateItem = async (itemId: string) => {
    try {
      ...
      // Create a MenuItemBase object for the API
      const newItemData = {
        name: duplicateName,
        description: item.description,
        category_id: item.category_id,
        ...
        default_spice_level: item.default_spice_level ?? undefined,
        dietary_tags: item.dietary_tags ?? undefined
      };

      const response = await brain.create_menu_item(newItemData);
      ...
      >
        ...
        <div className="flex gap-4 mb-4 p-5">
          ...
          <div className="flex-1 min-w-0">
            ...
            {!isEditing && enhancedItem.description && enhancedItem.description !== enhancedItem.menu_item_description && (
              <p
