...
export function PriceNumberInput({
  ...
  // Handle input change - update typing state AND call onChange immediately
  // This ensures the parent receives updates even if user clicks Save before blur
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (/^\d*\.?\d*$/.test(newValue) || newValue === '') {
      ...
      // Also call onChange immediately so parent gets the update
      // This fixes the issue where variant prices don't save if user clicks Save
      // without first clicking away from the price input
