...
// Local interfaces for refund types (not in generated brain types)
...
export function RefundManagementPanel({ className }: RefundManagementPanelProps) {
  ...
  const loadRefunds = async () => {
    try {
      ...
      const response = await (brain as any).list_refunds(params);
  ...
  const createRefund = async () => {
    try {
      ...
      const response = await (brain as any).create_refund(refundData);
  ...
  const getStatusBadge = (status: string) => {
    const variants = {
      completed: { color: QSAITheme.status.success, icon: CheckCircle, label: 'Completed' },
      processing: { color: QSAITheme.status.warning, icon: Clock, label: 'Processing' },
      failed: { color: QSAITheme.status.danger, icon: AlertCircle, label: 'Failed' },
      cancelled: { color: QSAITheme.text.muted, icon: X, label: 'Cancelled' }
    };
  ...
  return (
    <div className={className}>
      <Card 
        ...
        style={{ backgroundColor: `${QSAITheme.background.card}` }}
      >
              ...
              <DialogContent className="sm:max-w-[600px] bg-slate-900 border-slate-700">
                <DialogHeader>
                  <DialogTitle className="text-white flex items-center gap-2">
                    <DollarSign className="w-5 h-5" style={{ color: QSAITheme.purple.primary }} />
                    Process New Refund
                  </DialogTitle>
