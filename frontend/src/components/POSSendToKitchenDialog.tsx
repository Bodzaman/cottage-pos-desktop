...
// Note: DialogStep type is defined but 'main' state is handled inline rather than via useState
...
export function POSSendToKitchenDialog({
  ...
  const [currentStep, setCurrentStep] = useState<DialogStep>('main');
  ...
  const getItemCustomizations = (item: OrderItem) => {
    ...
    // Add modifiers (ModifierSelection items are already selected)
    if (item.modifiers && item.modifiers.length > 0) {
      item.modifiers.forEach(modifier => {
        const modText = modifier.price_adjustment > 0 ?
          `${modifier.name} (+£${modifier.price_adjustment.toFixed(2)})` :
          modifier.name;
        customizations.push(modText);
      });
    }
    
    // Add customizations (CustomizationSelection items are already selected)
    if (item.customizations && item.customizations.length > 0) {
      item.customizations.forEach(customization => {
        const custText = customization.price_adjustment > 0 ?
          `${customization.name} (+£${customization.price_adjustment.toFixed(2)})` :
          customization.name;
  ...
  const handlePaymentComplete = (tipSelection: TipSelection, _paymentResult?: PaymentResult) => {
