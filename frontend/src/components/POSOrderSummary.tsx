...
interface POSOrderItem extends Omit<OrderItem, 'modifiers'> {
  ...
  proteinType?: string;
  imageUrl?: string;
  // Override modifiers to use grouped structure with options
  modifiers: ModifierGroup[];
}
import { pendingPaymentService } from '../utils/pendingPaymentService';
import { colors, globalColors, QSAITheme, styles, effects } from '../utils/QSAIDesign';
import { useCustomerDataStore } from '../utils/customerDataStore';
import { kitchenService } from '../utils/kitchenService';
import { groupMultiCustomItems, MultiCustomGroup } from '../utils/multiCustomGrouping';
...
import { SchedulingData, formatSchedulingDisplay } from '../utils/schedulingTypes';
import { POSUnifiedPaymentModal } from './POSUnifiedPaymentModal';
import { PaymentResult } from './POSPaymentSelector';
import { TableStatus } from '../utils/tableTypes';
...
export interface POSOrderSummaryProps {
  ...
  onSaveUpdate?: (tableNumber: number, orderItems: POSOrderItem[]) => void;
  onClearOrder?: () => void;
  onQuantityChange: (index: number, newQuantity: number) => void;
  onCustomizeItem?: (index: number, item: POSOrderItem) => void;
...
const validateMinimumOrderValue = async (orderType: string, total: number) => {
  ...
  try {
    ...
    // GetPOSSettingsResponse has settings directly (no success property)
    if (data.settings) {
      const settings = data.settings as { delivery?: { minimum_order_value?: number } };
      const minimumOrderValue = settings.delivery?.minimum_order_value;
...
export function POSOrderSummary({
  ...
  // Group multi-custom items - cast to POSOrderItem for extended properties
  const { multiCustomGroups, regularItems: rawRegularItems } = groupMultiCustomItems(orderItems as unknown as OrderItem[]);
  const regularItems = rawRegularItems as unknown as POSOrderItem[];
  ...
  const handleEditMultiCustomPortion = (portionIndex: number, item: POSOrderItem) => {
  ...
  if (orderType === "DELIVERY") {
    // Use restaurant delivery settings from props (delivery_charge removed from posSettings)
  ...
  const handleStripeSuccess = (tipSelection: TipSelection, paymentResult?: MenuPaymentResult) => {
    setShowStripePayment(false);
    setPaymentProcessing(true);

    // Store tip selection for any follow-up processing
    setCurrentTipSelection(tipSelection);
      ...
      if (orderType === "DELIVERY") {
        ...
        if (result.method === 'CASH') {
          ...
          const orderData = {
            ...
            // Add scheduling fields based on order type (DELIVERY handled above)
  ...
  return (
    <div className="h-full min-h-0 flex flex-col rounded-lg overflow-hidden" style={{ 
        ...
        <div className="p-4">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-3">
              ...
              >
                {orderType === "DINE-IN" ? (
                  <Utensils className="h-6 w-6" style={{ color: QSAITheme.purple.primary }} />
              ...
              <motion.div layout className="space-y-3">
                ...
                {multiCustomGroups.map((group) => (
                  <MultiCustomOrderCard
                    ...
                    onCustomizeItem={onCustomizeItem ? (index, item) => onCustomizeItem(index, item as unknown as POSOrderItem) : undefined}
            ...
            <div className="space-y-3">
              ...
              <motion.button
                ...
                style={{
                  background: orderItems.length === 0
                    ? `rgba(18, 18, 18, 0.8)`
                    : `linear-gradient(135deg, ${colors.accent.turquoise} 0%, ${colors.accent.turquoise_light} 100%)`,
                  color: orderItems.length === 0 ? colors.text.disabled : colors.text.primary,
                  boxShadow: orderItems.length === 0 ? 'none' : `0 8px 16px rgba(0,0,0,0.2), 0 0 10px ${colors.accent.turquoise}40`,
                  cursor: orderItems.length === 0 ? 'not-allowed' : 'pointer',
                  borderRadius: '0.75rem',
                  border: `1px solid ${orderItems.length === 0 ? 'rgba(255, 255, 255, 0.05)' : `${colors.accent.turquoise}30`}`,
                  backdropFilter: 'blur(4px)',
                  padding: '0.875rem',
                  fontWeight: 600,
                  fontSize: '0.95rem',
      ...
      <MultiCustomDetailsModal
        ...
        onEditPortion={(index, item) => handleEditMultiCustomPortion(index, item as unknown as POSOrderItem)}
