...
// TODO: UltravoxCallModal component not used - removed import
...
import { NavigationConfig, NavigationContext } from 'utils/navigationConfig';
...
export default function OnlineOrders() {
  ...
  const {
    ...
    addItem,
    updateQuantity: updateItemQuantity, // ✅ FIX: Use updateQuantity (optional in CartState)
    removeItem,
    clearCart,
    getCartAge,
    isCartStale,
  ...
  const handleSearchChange = useCallback((query: string) => {
    ...
    searchDebounceTimer.current = setTimeout(() => {
      ...
      // Execute search
      // Note: searchMenuItems expects types.MenuItem[], we pass utils/menuTypes.MenuItem[] which is compatible at runtime
      const results = searchMenuItems(menuItems as any, {
        query,
    ...
    try {
      ...
      voiceCartItems.forEach(voiceItem => {
        ...
        if (menuItem) {
          ...
          // Add item with variant and customizations
          // CartState.addItem signature: (item, variant, quantity, customizations?, orderMode?, notes?)
          addItem(menuItem as any, variant, voiceItem.quantity, voiceItem.customizations || [], currentOrderMode, voiceItem.notes || '');
        }
      });
  ...
  const handleAddToCart = (item: MenuItem, variant: any, quantity: number, notes?: string, customizations?: any[]) => {
    ...
    // CartState.addItem signature: (item, variant, quantity, customizations?, orderMode?, notes?)
    addItem(item as any, variantWithPrice, quantity, customizations || [], currentOrderMode, notes || '');
  ...
  const handleVoiceOrderUpdate = (items: any[]) => {
    ...
    items.forEach(voiceItem => {
      if (voiceItem.menuItem && voiceItem.variant) {
        // CartState.addItem signature: (item, variant, quantity, customizations?, orderMode?, notes?)
        addItem(voiceItem.menuItem, voiceItem.variant, voiceItem.quantity || 1, voiceItem.customizations || [], currentOrderMode, voiceItem.notes || '');
      }
    });
    openCart();
  };
  ...
  return (
    <ErrorBoundary
      fallbackMessage="The online ordering page encountered an error. Please try again."
      onRetry={() => {
        // Log error with cart context on retry
        logError(
          new Error('OnlineOrders ErrorBoundary triggered'),
          null,
    ...
    >
      ...
      >
            ...
            <main className="lg:col-span-4 overflow-y-auto pb-20">
              ...
              {error ? (
                          ...
                          >
                            ...
                            {viewMode === 'gallery' ? (
                              <MenuGrid
                                ...
                                onItemSelect={handleMenuItemSelect as (item: MenuItem, variant?: ItemVariant) => void}
                                mode={currentOrderMode as 'delivery' | 'collection'}
                                isLoading={false}
                ...
                <div className="mb-12">
                  ...
                  {viewMode === 'gallery' ? (
                    <MenuGrid
                      menuItems={displayedItems as MenuItem[]}
                      onItemSelect={handleMenuItemSelect as (item: MenuItem, variant?: ItemVariant) => void}
                      mode={currentOrderMode as 'delivery' | 'collection'}
                      isLoading={false}
                      itemVariants={itemVariants}
        ...
        <CartRecoveryDialog
          ...
          menuItems={menuItems as any} // Type coercion: utils/menuTypes.MenuItem[] -> types.MenuItem[]
          cartPreview={{
            itemsCount: cartItems.length,
            totalValue: cartTotal,
            items: cartItems,
            savedDate: String(cartLastSavedAt || new Date().toISOString()),
            isStale: isCartStale(),
