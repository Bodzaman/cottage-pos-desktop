...
interface UnifiedAgentConfigResponse {
  id?: string;
  agent_name?: string;
  agent_role?: string;
  ...
  channel_settings?: {
    chat?: { system_prompt?: string; tone?: string[]; custom_instructions?: string };
...
interface PublishWizardConfigRequest {
  agent_name: string;
  agent_role: string;
  nationality: string;
  core_traits: string;
...
const AIStaffManagementHub: React.FC = () => {
  ...
  const validateAndSetError = (fieldName: keyof typeof validationRules, value: string): boolean => {
    ...
    setFieldErrors(prev => {
      ...
      if (result.isValid) {
        ...
        updated[fieldName as string] = result.error!;
      }
      return updated;
    });
  ...
  useEffect(() => {
    const loadConfiguration = async () => {
      try {
        ...
        // If config not found, auto-initialize with defaults
        if (!result.data) {
          await initializeUnifiedAgentConfig();
          result = await getUnifiedAgentConfig();
        }

        if (!result.success) {
          throw new Error(result.error || 'Failed to load agent config');
        }

        const agentData: UnifiedAgentConfigResponse = result.data as UnifiedAgentConfigResponse;
  ...
  const populateWizardState = async (config: UnifiedAgentConfigResponse) => {
    ...
    if (!voicePromptFromDB) {
      try {
        const data = await getActiveVoicePrompt();
        if (data.prompt) {
  ...
  const confirmPublish = async () => {
    try {
      ...
      // Call publish function directly via Supabase
      const result = await publishWizardConfig(publishRequest);
  ...
  const handleAvatarSelect = (asset: MediaItem) => {
    ...
    if (avatarUrl) {
      toast.success(`Avatar "${asset.friendlyName || asset.name}" selected!`);
    }
  ...
  const handleRemoveAvatar = async () => {
    try {
      setIsUploadingAvatar(true);
      await updateUnifiedAgentConfig({ agent_avatar_url: null });
  ...
  const handleGenerateChatPrompt = async () => {
    // Auto-generation requires LLM integration via Edge Function
    // For now, show message to enter prompt manually
    toast.info('Auto-generation is being updated. Please enter your chat prompt manually.', {
      duration: 4000,
    });
  };
  ...
  const handleGenerateVoicePrompt = async () => {
    // Auto-generation requires LLM integration via Edge Function
    // For now, show message to enter prompt manually
    toast.info('Auto-generation is being updated. Please enter your voice prompt manually.', {
      duration: 4000,
    });
  };
  ...
  const handleRevertToDefaultPrompt = async () => {
    try {
      // Fetch the default hardcoded prompt from Supabase
      const data = await getActiveVoicePrompt();
