...
export function useChatAnalytics() {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      const healthResponse = await brain.check_analytics_health();
      if (healthResponse.status !== 200) {
        throw new Error('Analytics service unavailable');
      }

      // Fetch real-time stats
      const statsResponse = await brain.get_real_time_stats();
      const conversationResponse = await brain.get_conversation_analytics();
