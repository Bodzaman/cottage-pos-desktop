...
export class OrderManagementService {
  ...
  private convertTableOrderToCompletedOrder(tableOrder: TableOrder, tableNumber: number, guestCount: number): CompletedOrder {
    ...
    return {
      ...
      order_number: tableOrder.orderId.slice(-6).toUpperCase(),
...
// Add a method to update order payment status
OrderManagementService.prototype.updateOrderPaymentStatus = async function(
  orderId: string,
  status: string,
  paymentInfo?: any
): Promise<boolean> {
  try {
    ...
    console.log(`Updating order ${orderId} payment status to ${status}`, paymentInfo);
    return true;
  } catch (error) {
