...
// Use LiveCallStatus from data-contracts but alias for internal use
type LiveCall = LiveCallStatus;

// Use AgentProfileOutput from data-contracts as the AgentProfile type
type AgentProfile = AgentProfileOutput;
...
const useAIVoiceStoreInternal = create<AIVoiceStore>((set, get) => ({
  ...
  loadSettings: async () => {
    set({ isLoading: true });
    try {
      const response = await brain.get_ai_voice_settings();
      const result = await response.json();

      // The API returns AIVoiceSettingsResponse with nested 'settings' property
      if (result.settings) {
        set({
          settings: {
            enabled: result.settings.enabled ?? false,
            custom_name: result.settings.custom_name ?? 'Cottage Tandoori Assistant',
            selected_agent_id: result.settings.selected_agent_id ?? '',
            auto_approve_orders: result.settings.auto_approve_orders ?? false,
            max_order_value: 50.00 // Not in the API response, use default
          },
          isLoading: false
        });
      } else {
  ...
  updateSettings: async (updates) => {
    ...
    try {
      ...
      // Save to backend using the correct API method
      const updatePayload: AIVoiceSettingsUpdate = {
        enabled: updatedSettings.enabled,
        custom_name: updatedSettings.custom_name,
        selected_agent_id: updatedSettings.selected_agent_id,
        auto_approve_orders: updatedSettings.auto_approve_orders
      };
      const response = await brain.update_ai_voice_settings(updatePayload);
...
const useAIVoiceMasterToggleInternal = create<AIVoiceMasterToggleStore>((set, get) => ({
  ...
  toggle: async (enabled) => {
    set({ isLoading: true });
    try {
      const response = await brain.toggle_ai_voice_assistant({ enabled });
  ...
  refresh: async () => {
    set({ isLoading: true });
    try {
      const response = await brain.get_voice_agent_status();
      const result = await response.json();
...
const useAIVoiceLiveCallsInternal = create<AIVoiceLiveCallsStore>((set, get) => ({
  ...
  refresh: async () => {
    set({ isLoading: true });
    try {
      const response = await brain.get_live_calls();
      const result = await response.json();

      const liveCalls = result.active_calls || [];
      set({
        liveCalls,
        totalActiveCalls: result.total_calls ?? liveCalls.length,
        isLoading: false
      });
    } catch (error) {
...
const useAIVoiceAgentsInternal = create<AIVoiceAgentsStore>((set, get) => ({
  ...
  loadAgents: async () => {
    set({ isLoading: true });
    try {
      const response = await brain.get_agent_profiles_endpoint();
      const data = await response.json();
      
      if (data.success && Array.isArray(data.agents)) {

        // 🛡️ PHASE 3: Filter to only include Ultravox-enabled agents
        // Note: ultravox_agent_id may be present in the API response as an extended field
        const ultravoxAgents = data.agents.filter((agent: AgentProfile & { ultravox_agent_id?: string }) => {
          const ultravoxId = agent.ultravox_agent_id;
          return ultravoxId && ultravoxId.trim() !== '';
        });
...
const useAIVoiceConnectionInternal = create<AIVoiceConnectionStore>((set, get) => ({
  ...
  test: async () => {
    set({ isLoading: true });
    try {
      const response = await brain.test_ai_voice_connection();
