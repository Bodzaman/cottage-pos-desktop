...
import { MenuItemFormData, MenuItemVariant } from './masterTypes';
...
export function getFormSteps(
  ...
  if (!configuration) {
    // No configuration: show configuration step first
    return [
      {
        id: 'configuration',
        title: 'Item Type & Pricing',
        description: 'Choose item type and pricing structure',
        required: true,
        status: 'not-started',
        order: 0,
        icon: Settings2
...
export function calculateStepStatus(
  ...
  switch (stepId) {
    case 'configuration': {
      // Configuration step is complete if configuration object exists with both values
      if (configuration && configuration.itemType && configuration.pricingMode) {
        return 'complete';
      }
      return 'incomplete';
    }
    ...
    case 'configuration': {
      if (configuration && configuration.itemType && configuration.pricingMode) {
        const typeNames: Record<string, string> = {
          food: 'Food',
          drinks_wine: 'Drinks & Wine',
          coffee_desserts: 'Coffee & Desserts'
        };
        const modeNames: Record<string, string> = {
          single: 'Single Price',
          variants: 'Multiple Variants'
        };
        return `${typeNames[configuration.itemType]} • ${modeNames[configuration.pricingMode]}`;
      }
      return null;
    }
