...
class PaymentNotificationService {
  ...
  async getNotifications(limit: number = 5): Promise<any[]> {
    try {
      ...
      if (now - this.lastFetchTime > 10000) {
        ...
        try {
          const response = await (brain as any).get_webhook_notifications({ limit });
        ...
        if (!successfulFetch && !this.permissionIssueDetected) {
          try {
            const response = await brain.get_payment_notifications_v2({ limit });
  ...
  async createTestNotification(orderId: string): Promise<boolean> {
    try {
      ...
      const response = await (brain as any).test_notification({ orderId });
  ...
  private showNotificationToast(notification: any) {
    ...
    switch (notification.event_type) {
      ...
      default:
        ...
        description = `Update for order #${notification.order_id}`;
        variant = 'default';
    }
  ...
  async markAsProcessed(orderIds: string[]): Promise<boolean> {
    try {
      ...
      let response = await (brain as any).mark_webhook_notifications_processed({ notification_ids: orderIds.map((_, i) => i) });
      
      if (!response.ok) {
        // Fall back to payment notifications endpoint
        response = await brain.mark_notifications_processed_v2({ notification_ids: orderIds.map((_, i) => i) });
      }
