...
export function determinePricingMode(
  ...
  if (hasVariants) {
    // ✅ Check mode-specific prices instead of just base price
    const validVariants = itemVariants.filter(v => {
      const variantPrice = getVariantPrice(v, mode);
...
export function getVariantPriceRange(
  ...
  // ✅ Filter out zero prices (invalid pricing)
  const validPrices = prices.filter(p => p > 0);

  // If no valid prices, return 0 (will trigger "Price not set")
...
export function getItemDisplayPrice(
  ...
  switch (mode) {
    case 'DINE-IN':
      basePrice = item.price_dine_in ?? item.base_price ?? 0;
      break;
    case 'DELIVERY':
      basePrice = item.price_delivery ?? item.base_price ?? 0;
      break;
    case 'COLLECTION':
    case 'WAITING':
    default:
      basePrice = item.price_takeaway ?? item.base_price ?? 0;
  }
