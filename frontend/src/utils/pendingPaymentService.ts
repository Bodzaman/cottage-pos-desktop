...
class PendingPaymentService {
  ...
  addPendingOrder(order: Omit<PendingPaymentOrder, 'id' | 'timestamp'>): string {
    ...
    toast.success('Order added to pending queue', {
      description: `Order ${orderId.slice(-6)} is waiting for payment`
    });
  ...
  /**
   * Mark kitchen ticket as printed for an order
   */
  markKitchenTicketPrinted(orderId: string): boolean {
    const orders = this.getPendingOrders();
    ...
    order.kitchenTicketPrinted = true;
    this.saveOrders(orders);
    this.notifyListeners(orders);
    return true;
  }
  ...
  /**
   * Mark customer receipt as printed for an order
   */
  markCustomerReceiptPrinted(orderId: string): boolean {
    const orders = this.getPendingOrders();
    const order = orders.find(o => o.id === orderId);
    ...
    order.customerReceiptPrinted = true;
    this.saveOrders(orders);
    this.notifyListeners(orders);
    return true;
  }
