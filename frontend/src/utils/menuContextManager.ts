...
export class MenuContextManager {
  ...
  /**
   * Generate unique version string for menu context
   */
  ...
  /**
   * Handle menu update with debouncing
   */
  private handleMenuUpdate(): void {
    ...
    this.updateTimeout = setTimeout(() => {
      this.refreshMenuContext().catch(error => {
        console.error('Menu update refresh failed:', error);
      });
    ...
    for (const item of this.menuContext.items) {
      ...
      if (overallScore >= options.minScore) {
        results.push({
          ...
          confidence: this.calculateConfidence(overallScore, item.popularity)
        });
      }
    }
