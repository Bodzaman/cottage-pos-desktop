...
import { loadPOSBundle, startRealtimeSubscriptionsIfNeeded } from './realtimeMenuStore';
  ...
  useEffect(() => {
    const initializePOSDesktop = async () => {
      try {
        ...
        // ✅ CRITICAL FIX: Update filtered items AFTER network bundle loads
        // This ensures the UI reflects the fresh network data, not stale cache
        // Without this, filteredMenuItems may contain old cache data while menuItems has fresh data
        store.updateFilteredItems();
        if (isDev) console.log('🔄 [POSDesktop] Filtered items updated with network data');
        ...
        // Reduced from 15s to ensure admin publish updates reach POS quickly
        setTimeout(() => {
          if (isDev) console.log('⏰ [POSDesktop] Starting real-time subscriptions...');
