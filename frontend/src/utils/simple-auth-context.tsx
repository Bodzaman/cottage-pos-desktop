...
export function SimpleAuthProvider({ children }: SimpleAuthProviderProps) {
  ...
  const createList = async (listName: string) => {
    ...
    try {
      ...
      if (!data.success) {
        return { error: { message: data.message || 'Failed to create list' }, list: null };
      }
  ...
  const renameList = async (listId: string, newName: string) => {
    ...
    try {
      const response = await brain.rename_favorite_list({ list_id: listId, new_name: newName, customer_id: profile.id });
      const data = await response.json();
  ...
  const deleteList = async (listId: string) => {
    ...
    try {
      const response = await brain.delete_favorite_list({ list_id: listId, customer_id: profile.id });
      const data = await response.json();
