...
export function useMenuItemForm({
  ...
  const [selectedCategory, setSelectedCategory] = useState<string>(menuItem?.category_id || initialData?.category_id || '');
  ...
  const autoPopulateOrdering = async () => {
    ...
    try {
      ...
      const displayOrderResponse = await brain.get_next_item_display_order({ 
      ...
      if (displayOrderResponse.ok) {
        ...
        setOrderContext({
          ...
          nextOrder: displayOrderData.next_order
        });
      }
    } catch (error) {
    ...
    try {
      ...
      if (!pricingValidation.isValid) {
        toast.error('Pricing validation failed', {
          description: pricingValidation.errors.join(', ')
        });
        return; // Don't submit
      }
