...
import { updateMediaAssetUsage } from './mediaAssetUtils';

// Local type definitions to avoid tsconfig brain exclusion issues
// These match the brain/data-contracts types
...
interface OptimizedMediaResponse {
  success: boolean;
  message: string;
  media_id: string;
...
export interface MediaFilterOptions {
  ...
  linkedOnly?: boolean;    // Maps to linked_only
  unlinkedOnly?: boolean;  // Maps to unlinked_only
}
...
export const cleanupOrphanedMedia = async (deleteFiles: boolean = false): Promise<{success: boolean, count: number}> => {
  try {
    // Note: cleanup_orphaned_media takes no parameters - delete_files option not supported via API
    const response = await brain.cleanup_orphaned_media();
    const data = await response.json() as { success?: boolean; orphaned_assets_found?: number };
...
const fetchMediaLibraryFallback = async (filterOptions?: MediaFilterOptions): Promise<MediaItem[]> => {
  try {
    // Map to GetMediaLibraryParams: category, subcategory, tags, page, page_size
    const backendParams = {
      category: filterOptions?.type,
      tags: filterOptions?.tag || (filterOptions?.tags && filterOptions.tags.length > 0 ? filterOptions.tags[0] : undefined),
      page: filterOptions?.offset ? Math.floor(filterOptions.offset / (filterOptions?.limit || 20)) + 1 : 1,
      page_size: filterOptions?.limit || 20
    };

    // Use the original media_assets API as fallback
    const response = await brain.get_media_library(backendParams);
    const data = await response.json() as { success: boolean; assets?: any[]; message?: string; error?: string };
...
export const fetchMetadata = async (): Promise<MediaMetadata> => {
  try {
    // Use the new enhanced API with default params
...
export const updateMediaAsset = async (
  ...
  try {
    ...
    const assetResponse = await brain.get_media_asset({ assetId });
    const assetData = await assetResponse.json() as { success?: boolean; asset?: any; message?: string };
    ...
    // Update the asset
    const response = await brain.update_media_asset({ assetId }, updatePayload);
    const responseData = await response.json() as { success?: boolean; asset?: any; message?: string };
...
export const bulkUpdateTags = async (assetIds: string[], tags: string[]): Promise<boolean> => {
  try {
    const response = await brain.bulk_update_media_tags({ asset_ids: assetIds, tags });
...
export const bulkDeleteAssets = async (assetIds: string[]): Promise<boolean> => {
  try {
    // Delete each asset individually since there's no bulk delete API for media
