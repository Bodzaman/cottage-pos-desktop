...
import { CanvasElement, FontFamily, FontWeight, TextAlign } from './visualTemplateTypes';

// Extended element types for the visual designer palette
// These extend beyond the base ElementType to support richer receipt elements
export type VisualDesignerElementType =
  | 'logo'
  | 'business_info'
  | 'order_header'
  | 'customer_info'
...
export interface VisualDesignerElementStyle {
...
export interface PaletteElement {
  id: string;
  type: VisualDesignerElementType;
  name: string;
  description: string;
  icon: string;
  ...
  defaultStyle: VisualDesignerElementStyle;
}
...
// Extended CanvasElement type for visual designer that supports extended element types
export interface VisualDesignerCanvasElement extends Omit<CanvasElement, 'type' | 'style'> {
  type: VisualDesignerElementType;
  style: VisualDesignerElementStyle;
}
...
export function calculateAutoPosition(
  existingElements: VisualDesignerCanvasElement[],
  _newElementHeight: number = 40
): { x: number; y: number } {
...
export function validateElementPosition(
  element: VisualDesignerCanvasElement,
 ...
 */
export function sortElementsByPosition(elements: VisualDesignerCanvasElement[]): VisualDesignerCanvasElement[] {
 ...
 */
export function getElementBounds(element: VisualDesignerCanvasElement) {
  return {
    left: element.x,
    top: element.y,
 ...
 */
export function elementsOverlap(element1: VisualDesignerCanvasElement, element2: VisualDesignerCanvasElement): boolean {
 ...
 */
export function generateElementPreview(element: VisualDesignerCanvasElement, maxLength: number = 50): string {
