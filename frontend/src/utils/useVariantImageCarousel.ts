...
export function useVariantImageCarousel(
  ...
  externalIsPaused: boolean = false
): {
  currentImage: string;
  currentIndex: number;
  isLoading: boolean;
  ...
  // Combined pause state (internal OR external)
  const isPaused = internalPaused || externalIsPaused;
  ...
  return {
    currentImage: variantImages[currentIndex] || '',
    currentIndex,
    isLoading,
    isPaused,
    goToPrevious,
