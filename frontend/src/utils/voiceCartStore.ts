...
export const useVoiceCartStore = create<VoiceCartState>()(subscribeWithSelector((set, get) => ({
  ...
  // Voice Search Actions
  setSearchResults: (results) => set({ searchResults: results, showSearchResults: results.length > 0 }),
  ...
  performItemLookup: async (searchTerm: string) => {
    ...
    try {
      const response = await (brain as any).voice_search_menu_items({
  ...
  refreshFromTools: async (sessionId: string, userId?: string) => {
    try {
      const response = await (brain as any).get_voice_cart({
  ...
  addToCartViaTools: async (itemCode: string, quantity: number, userId?: string) => {
    ...
    try {
      // TODO: Update Brain SDK to include add_to_voice_cart endpoint
      const response = await (brain as any).add_to_voice_cart({
        session_id: sessionId,
        item_code: itemCode,
        quantity: quantity,
        user_id: userId
  ...
  initializeSession: async (userId?: string) => {
    try {
      // TODO: Update Brain SDK to include create_voice_session endpoint
      const response = await (brain as any).create_voice_session({
        user_id: userId
      });
