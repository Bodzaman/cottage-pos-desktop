...
export const linkMediaToMenuItem = async (
  menuItemId: string,
  assetId: string,
  mediaType: 'primary' | 'secondary' = 'primary',
  updateUsage: boolean = true
) => {
  try {
    const response = await (brain as any).link_media_to_menu_item({
      item_id: menuItemId,
      asset_id: assetId,
      item_type: 'menu_item',
      link_type: mediaType
...
export const getMenuMediaStatus = async () => {
  try {
    const response = await brain.get_menu_media_status_v3();
    return await response.json();
  } catch (error) {
...
export const getMenuMediaRelationships = async (limit: number = 100, offset: number = 0) => {
  try {
    const response = await brain.get_menu_media_relationships_v3({ limit, offset });
    return await response.json();
  } catch (error) {
...
export const fixMissingMediaReferences = async () => {
  try {
    const response = await brain.fix_missing_media_references_v3();
    return await response.json();
  } catch (error) {
...
export const cleanupOrphanedMedia = async (deleteOrphaned: boolean = false) => {
  try {
    const response = await brain.cleanup_orphaned_media_v3({ delete_orphaned: deleteOrphaned });
    return await response.json();
  } catch (error) {
