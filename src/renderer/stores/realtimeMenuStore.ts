 ...
 * 
 * ADAPTED FOR ELECTRON: This store provides real-time synchronization between AdminMenu and POS
 ...
 * - Maintained all Zustand functionality
 */
...
export const useRealtimeMenuStore = create<MenuStoreState>((set, get) => ({
  ...
  refreshData: async () => {
    try {
      ...
      // ELECTRON: Replace brain.get_menu_with_ordering() with apiClient call
      const result = await apiClient.getMenuWithOrdering();
  ...
  triggerCorpusSync: async () => {
    ...
    try {
      ...
      // ELECTRON: Replace brain.sync_menu_corpus with apiClient call
      const result = await apiClient.syncMenuCorpus({ force: true });
  ...
  fetchSetMeals: async () => {
    try {
      ...
      // ELECTRON: Replace brain.list_set_meals with apiClient call
      const setMeals = await apiClient.listSetMeals({ active_only: true });
...
export const realtimeMenuStore = useRealtimeMenuStore.getState;

// Export store actions for external usage
export const menuStoreActions = {
  initialize: () => useRealtimeMenuStore.getState().initialize(),
  refreshData: () => useRealtimeMenuStore.getState().refreshData(),
  forceFullRefresh: () => useRealtimeMenuStore.getState().forceFullRefresh(),
  setSelectedParentCategory: (categoryId: string | null) => useRealtimeMenuStore.getState().setSelectedParentCategory(categoryId),
  setSelectedMenuCategory: (categoryId: string | null) => useRealtimeMenuStore.getState().setSelectedMenuCategory(categoryId),
  setSearchQuery: (query: string) => useRealtimeMenuStore.getState().setSearchQuery(query)
};
